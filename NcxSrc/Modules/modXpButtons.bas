...
Option Explicit
'~modXpButtons.bas;
'Compile VB programs with XP-Style buttons
'**************************************************************************************
...
'-----------------------------
' Step 1: In the startup form for your program, call the FormInialize() subroutine from
...
Public Sub FormInitialize()
  ...
  If GetVersionEx(OSV) Then                               'get OS info
    If OSV.dwPlatformId = VER_PLATFORM_WIN32_NT Then      'WinNT type?
      If OSV.dwMajorVersion >= 5 Then                     'XP series (or later)?
        ...
        If Len(Dir$(FilePath)) = 0 Then                   'file exists?
          FileNum = FreeFile()                            'so, so we are going to build it
          On Error Resume Next
          Open FilePath For Output As #FileNum            'open for writing
          If Err.Number Then Exit Sub                     'error (on CD?), so ignore
          Print #FileNum, _
            ...
            "<assemblyIdentity version=""1.0.0.0"" processorArchitecture=""x86"" name=""prjThemed"" type=""win32"" />" & vbCrLf & _
            "<dependency>" & vbCrLf & _
...
'
        With ComCtls
          ...
          .dwICC = ICC_INTERNET_CLASSES           'Tell system "I WANT XP Buttons"
        End With
