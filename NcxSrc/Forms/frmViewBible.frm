...
Begin VB.Form frmViewBible 
   Caption         =   "View Bible"
   ClientHeight    =   7620
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   11160
   Icon            =   "frmViewBible.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   ScaleHeight     =   7620
   ScaleWidth      =   11160
   ...
   Begin VB.ComboBox cboBook 
      ...
      EndProperty
      Height          =   315
      ItemData        =   "frmViewBible.frx":0E54
      Left            =   720
      List            =   "frmViewBible.frx":0E56
   ...
   End
End
Attribute VB_Name = "frmViewBible"
...
Private UserBible As String

Private Books() As String           'Book database info
Dim Fso As FileSystemObject         'File I/O object
Dim Ts As TextStream                'File read/write object
Private bCancel As Boolean          'Cancel flag
Private cVLight As Long             'text background color
Private PNotesColor As Long         'color to print personal notes as
...
' add some color
'
  cVLight = RGB(235, 235, 235)          'light shade of gray
  Me.RichTextBox1.BackColor = cVLight
  PNotesColor = CLng(GetSetting("NCXlate", "Settings", "PNotesColor", CStr(vbBlue)))
'
' show business
'
...
' get user bible
'
  Txt = GetSetting("NCXlate", "Settings", "BibleBase", vbNullString)
...
'
  ...
  If Err.Number <> 0 Then
    ...
    MsgBox "Cannot find location of Books.txt database. Aborting.", _
           vbOKOnly Or vbExclamation, "DB File Not Found"
    Unload Me
    Exit Sub
  End If
  Books = Split(Ts.ReadAll, vbCrLf)
  Ts.Close
'
' Get list of Books/chapters/verses
'
  On Error Resume Next
  ...
  If Err.Number <> 0 Then
    Screen.MousePointer = vbDefault
    Me.RichTextBox1.Text = vbNullString
    MsgBox "Cannot find location of Greek.txt database. Aborting.", _
           vbOKOnly Or vbExclamation, "DB File Not Found"
    Unload Me
    Exit Sub
  End If
...
'
  For Idx = 0 To UBound(Ary) - 1
    Me.lstGrk.AddItem Left$(Ary(Idx), 6)
  Next Idx
'
' load the bible file
'
  UserBible = GetSetting("NCXLate", "Settings", "SaveBible", vbNullString)
...
Private Sub Form_Resize()
  ...
  Me.RichTextBox1.Height = Me.ScaleHeight - Me.StatusBar1.Height - Me.RichTextBox1.Top
...
' Purpose           : Set up manifest file (if XP) if needed
'*******************************************************************************
...
Private Sub SetVerseCount()
  ...
  i = FindExactMatch(lstGrk, S & "01")   'find verse 1
  With Me.cboVerse
    .Clear                                  'remove any old data
    Do
      .AddItem CStr(.ListCount + 1)         'add an item
    Loop While Left$(Me.lstGrk.List(i + .ListCount), 4) = S
...
Private Sub cmdFindInText_Click()
  ...
  Sch = InputBox("Enter word or phrase to find:", "Search For Text", LastSch)
  If Len(Sch) = 0 Then Exit Sub
  LastSch = Sch
  With Me.RichTextBox1
    Text = .Text
    Idx = .SelStart + .SelLength + 1
    Idx = InStr(Idx, Text, Sch, vbTextCompare)    'find non-case sensetive match
    If Idx <> 0 Then
      .SelStart = Idx - 1
      .SelLength = Len(Sch)
...
Private Sub FindBkChVs(Text As String, ByVal Index As Long)
  ...
  For B = 1 To 27
    ...
    S = Ary(3) & ", Chapter "             'get book title
    Ccnt = CLng(Ary(4))                   'get chapter count
    For C = 1 To Ccnt
      T = S & CStr(C)                     'get test string for book and chapter heading
      Idx = InStr(oIdx, Text, T)
      If Idx > Index Then
        Me.cboBook.ListIndex = B - 1      'found our sought book
'
...
'
        With Me.cboVerse
          ...
          i = FindExactMatch(Me.lstGrk, T & "01")   'find verse 1 in header list
          Do
            Vcnt = Vcnt + 1                         'find consecutive verses
            .AddItem CStr(Vcnt)                     'add item to list
          Loop While Left$(Me.lstGrk.List(i + Vcnt), 4) = T
        End With
'
' find target Verse
'
...
'******************************************************************************
' This routine, using frmMsgBox.frm, displays a message box that is customizable.
